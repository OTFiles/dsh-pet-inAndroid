...
class PetConfig(private val ctx: Context) {
    ...
    // ================ 读取 ================
    fun character() = getString(K_CHARACTER, DEFAULT_CHARACTER)
    fun scale() = getDouble(K_SCALE, 0.72)
    fun facing() = getString(K_FACING, "left")
    fun onTop() = getBool(K_ON_TOP, true)
    fun noMove() = getBool(K_NO_MOVE, false)
    fun lockPosition() = getBool(K_LOCK_POSITION, false)
    fun shiftDrag() = getBool(K_SHIFT_DRAG, false)
    fun dragPhysics() = getBool(K_DRAG_PHYSICS, false)
    fun petOpacity() = getInt(K_PET_OPACITY, 100).coerceIn(10, 100)
    fun playbackSpeed() = getDouble(K_PLAYBACK_SPEED, 1.0).coerceIn(0.5, 3.0)
    fun animGapSeconds() = getDouble(K_ANIM_GAP_SEC, 0.0).coerceIn(0.0, 3600.0)
    fun clickSound() = getBool(K_CLICK_SOUND, true)
    fun clickShowBalance() = getBool(K_CLICK_SHOW_BALANCE, false)
    fun clickShowSelfTalk() = getBool(K_CLICK_SHOW_SELF_TALK, false)

    fun selfTalkEnabled() = getBool(K_SELF_TALK, false)
    fun selfTalkMin() = getInt(K_SELF_TALK_MIN, 20)
    fun selfTalkMax() = getInt(K_SELF_TALK_MAX, 60)
    fun selfTalkDuration() = getDouble(K_SELF_TALK_DURATION, 3.2)
    fun selfTalkTexts(): Set<String> = getStringSet(K_SELF_TALK_TEXTS, defaultSelfTalkTexts())
    fun selfTalkBubbleStyle() = getString(K_SELF_TALK_BUBBLE_STYLE, "classic_top")

    fun blurEnabled() = getBool(K_BLUR, false)
    fun hideFromRecents() = getBool(K_HIDE_RECENTS, true)
    fun autoStart() = getBool(K_AUTO_START, false)
    fun batteryOpt() = getBool(K_BATTERY_OPT, false)
    fun overlayPermissionGranted() = getBool(K_OVERLAY_PERM, false)
    fun petRunning() = getBool(K_PET_RUNNING, false)

    fun chatProviderName() = getString(K_CHAT_NAME, "DeepSeek")
    fun chatBaseUrl() = getString(K_CHAT_BASE_URL, "https://api.deepseek.com")
    fun chatPath() = getString(K_CHAT_PATH, "/v1/chat/completions")
    fun chatModel() = getString(K_CHAT_MODEL, "deepseek-chat")
    fun chatApiKey() = getString(K_CHAT_API_KEY, "")
    fun chatTemperature() = getDouble(K_CHAT_TEMPERATURE, 0.7).coerceIn(0.0, 2.0)
    fun chatMaxTokens() = getInt(K_CHAT_MAX_TOKENS, 2048)
    fun chatTimeout() = getInt(K_CHAT_TIMEOUT, 60)
    fun chatVerifySsl() = getBool(K_CHAT_VERIFY_SSL, true)
    fun balanceRefreshMinutes() = getInt(K_BALANCE_REFRESH_MIN, 0)

    fun quickLaunch(): List<Pair<String, String>> =
        getStringSet(K_QUICK_LAUNCH, emptySet())
            .mapNotNull { it.split("|", limit = 2).let { p -> if (p.size == 2) p[0] to p[1] else null } }

    fun instanceCount() = getInt(K_INSTANCE_COUNT, 0)
